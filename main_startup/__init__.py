...
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s - [UB] - %(levelname)s - %(message)s",
)
...
if not Config.STRINGSESSION:
    logging.error("No String Session Found! UB is Exiting!")
    quit(1)

if not Config.API_ID:
    logging.error("No Api-ID Found! UB is Exiting!")
    quit(1)

if not Config.API_HASH:
    logging.error("No ApiHash Found! UB is Exiting!")
    quit(1)

if not Config.LOG_GRP:
    logging.error("No Log Group ID Found! UB is Exiting!")
    quit(1)
